...
def load_transport_from_json(path=None):
    path = Path(path) if path is not None else Path(__file__).resolve().parent / "input.json"
    ...
    if cost.shape != (m, n):
        raise ValueError(
            f"cost must be {m}x{n} (rows = supply, cols = demand), got {cost.shape}"
        )
...
if __name__ == "__main__":
    supply, demand, cost = load_transport_from_json()
    transport_task(supply, demand, cost)